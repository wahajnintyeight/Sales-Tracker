...
function displaySelectedGauge(index, id, calls, totalCalls, fupCalls, fupTotalCalls) {

    console.log("I receive ID:", id)
    var target = document.getElementById('calls-gauge' + id); // your canvas element

    var goalCalls = totalCalls;
    var goalCallLabel = [0, Math.round(goalCalls / 5), Math.round(goalCalls / 5) * 2, Math.round(goalCalls / 5) * 3, Math.round(goalCalls / 5) * 4, goalCalls];//;
    var goalCallLabelFormula = Math.round(goalCalls / 5)
    var opts = {
        angle: 0, // The span of the gauge arc
    ...
    gauge.maxValue = totalCalls; // set max gauge value
    gauge.setMinValue(0); // Prefer setter over gauge.minValue = 0
    gauge.animationSpeed = 32; // set animation speed (32 is default value)


    if (typeof calls !== "undefined") {
        gauge.set(calls); // set actual value
    } else {
        gauge.set(0); // set actual value
    }
    ...
    */
    ...
    var goalCalls = fupTotalCalls;
    console.log("goal calls", goalCalls)
    var goalCallLabel = [0, Math.round(goalCalls / 5), Math.round(goalCalls / 5) * 2, Math.round(goalCalls / 5) * 3, Math.round(goalCalls / 5) * 4, goalCalls];//;
    var goalCallLabelFormula = Math.round(goalCalls / 5)
    ...
    var target = document.getElementById('fup-calls-gauge' + id); // your canvas element
    var gauge = new Gauge(target).setOptions(opts); // create sexy gauge!
    gauge.maxValue = goalCalls; // set max gauge value
    ...
    if (fupCalls != 0) {
        // console.log(fupCalls[index]);
        gauge.set(fupCalls); // set actual value
    } else {
        gauge.set(0); // set actual value
    }
}